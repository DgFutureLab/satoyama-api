...
if [ -e "$HOME/$envname/bin/activate" ]; then
	...
	if [ $? -eq 0 ]; then
		...
		echo "${ERROR} Could not activate new Python environment. Exitting..."
		exit 1
	fi
else
...
if [ -e "$apifolder/requirements.txt" ]; then
	...
	if [ $? -eq 0 ]; then
		...
		echo "${ERROR} Failed to install python packages. Exitting..."
		exit 1
	fi
else
...
if [ -e "$apifolder/db_config_sample.yml" ]; then
	if [ -e "$apifolder/db_config.yml" ]; then
		...
		echo "${SUCCESS} Copied db_config_sample.yml to db_config.yml"
